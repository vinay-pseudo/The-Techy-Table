...
// get all Foods
export const getRecommendedFoods = (result) => {
    db.query("SELECT * FROM food", (err,results)=> {
        if (err){
            console.log(err);
            result(err,null);
            ...
            while(set.length < 5) {
                let idx = Math.floor(Math.random()*(results.length - 1));
                if(set.includes(idx)) continue;
                set.push(idx);
                newresults.push(results[idx]);
            }

            // console.log("fadfa", newresults);
            result(null,newresults);
        }
    });
};
